...
def load_config(path: Path | str = "config.yaml") -> dict:
    ...
    for tpl_key in ("ask_address_templates", "ask_address_no_product_templates", "comment_fallback_templates", "comment_order_done_templates", "oos_templates"):
        val = messages.get(tpl_key)
        if isinstance(val, list):
            messages_out[tpl_key] = [str(s) for s in val]
    for tpl_key in ("deposit_template", "oos_line_format"):
        val = messages.get(tpl_key)
